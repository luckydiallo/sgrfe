...
queryProfesseurs(): Observable<HttpResponse<IUser[]>> {
  return this.http.get<IUser[]>(`${this.resourceUrl}/professeurs`, {
    observe: 'response',
  }).pipe(
    tap(res => console.log('Professeurs reçus du backend:', res.body))
  );
}



queryDirecteur(): Observable<HttpResponse<IUser[]>> {
  return this.http.get<IUser[]>(`${this.resourceUrl}/directeur`, {
    observe: 'response',
  }).pipe(
    tap(res => console.log('Directeur reçu du backend:', res.body))
  );
}
