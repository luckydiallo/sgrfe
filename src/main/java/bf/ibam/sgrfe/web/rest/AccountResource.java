package bf.ibam.sgrfe.web.rest;

import bf.ibam.sgrfe.repository.EtudiantRepository;
import bf.ibam.sgrfe.repository.UserRepository;
import bf.ibam.sgrfe.security.SecurityUtils;
import bf.ibam.sgrfe.service.MailService;
import bf.ibam.sgrfe.service.UserService;
import bf.ibam.sgrfe.service.dto.AdminUserDTO;
import bf.ibam.sgrfe.service.dto.PasswordChangeDTO;
import bf.ibam.sgrfe.web.rest.errors.*;
import bf.ibam.sgrfe.web.rest.vm.KeyAndPasswordVM;
import bf.ibam.sgrfe.web.rest.vm.ManagedUserVM;
...
public class AccountResource {
    ...
    public AccountResource(UserRepository userRepository, UserService userService, MailService mailService, EtudiantRepository etudiantRepository) {
    ...
    public Mono<AdminUserDTO> getAccount() {
        return userService.getUserWithAuthorities()
            .flatMap(user -> {
                ...
                if (dto.getAuthorities().contains("ROLE_ETUDIANT")) {
                    return etudiantRepository
                        .findById(user.getId())
                        .map(etudiant -> {
                            dto.setNiveauEtude(etudiant.getNiveauEtude());
                            dto.setFiliereId(etudiant.getFiliereId());
                            return dto;
                        });
                }
